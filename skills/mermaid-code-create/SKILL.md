...
在mindmap代码中,若节点包含了如括号 `()`、等号 `=`、斜杠 `/`、方括号 `[]` 等特殊字符,会被 Mermaid 误解析为节点定义语法,需要将这类节点全部用双引号包裹,例如：
- 错误: PE(pos,2i)=sin(pos/10000^(2i/d_model))
- 正确为: ["PE(pos,2i)=sin(pos/10000^(2i/d_model))"]
- 错误: [C: ]更大模型更好
- 正确为: [C: 更大模型更好]
